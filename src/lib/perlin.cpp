...
// Last update Sat May 27 17:57:21 2017 Thomas ARBONA
*/
#include <cstdio>
#include <cmath>
#include "vector.hpp"
...
rt::Perlin::dot3(const char a[], float x, float y, float z)
{
  return (a[0] * x + a[1] * y + a[2] * z);
}

float __attribute__ ((const))
rt::Perlin::lerp(float a, float b, float t)
{
  return ((1 - t) * a + t * b);
}

float __attribute__ ((const))
rt::Perlin::fade(float n)
{
  return (n * n * n * (n * (n * 6 - 15) + 10));
}

float __attribute__((pure))
rt::Perlin::getValue2(unsigned int gi[8], rt::Vector3 r, rt::Vector3 f)
{
  float n[8], nx[4], nxy[2];
  int index;

  index = -1;
  while (++index != 8)
    n[index] = Perlin::dot3(g_grad3[gi[index]], r.x - ((index >> 2) & 1),
  ...
  index = -1;
  while (++index != 4)
    nx[index] = Perlin::lerp(n[index], n[4 + index], f.x);
  index = -1;
  while (++index != 2)
    nxy[index] = Perlin::lerp(nx[index], nx[2 + index], f.y);
  return (Perlin::lerp(nxy[0], nxy[1], f.z));
}

float __attribute__((pure))
rt::Perlin::getValue(float x, float y, float z)
{
  rt::Vector3 g, r, f;
  unsigned int gi[8];
  int index;
  ...
  return (Perlin::getValue2(gi, r, f));
}
